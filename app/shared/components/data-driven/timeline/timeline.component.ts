...
@Component({
	selector: 'app-data-driven-timeline',
	templateUrl: './timeline.component.html',
	styleUrls: ['./timeline.component.scss'],
	encapsulation: ViewEncapsulation.None
})
export class DataDrivenTimelineComponent implements OnInit {
	ABO_ICONS = ABO_ICONS;
    ...
    views: any[] = [
        { type: "timelineDayView", 
            ...
            appointmentHeight: 15,
            timeSlotWidth: 60
        },
        { type: 'timelineWeekView', 
            text: "Week",
            workTime:{ fromDayOfWeek: 1, toDayOfWeek: 5, fromHour: WORKING_START_HOUR, toHour: WORKING_END_HOUR },
            appointmentHeight: 12,
            timeSlotWidth: 60
        },
    ...
    setView(typeview: TypeView){
        ...
        if (typeview == TypeView.Day) {
            let view = 'timelineDayView'
            return view
        } 
        if (typeview == TypeView.Week) {
            let view = 'timelineWeekView'
            return view
        } 
        if (typeview == TypeView.Month) {
            let view = 'timelineMonthView'
            return view
        } else {
            let view = 'weekView'
            return view
